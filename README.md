...
mkdir {{folder}}
```
3. Node installation (**reqeust 'sketch.json' file to sketchnet developer )
```shell
geth --datadir {{folder}}/ init masnet.json 
```
4. Start Node && customize RPC Port and directory of block data
```shell
geth --datadir node/ --ipcpath {{folder}}/{{folder}}.ipc --port 30301 --networkid 220401 --syncmode full --nodiscover <br>
```
...
5. join to masnet (** reqeust 'enode infomaiton' to sketchnet developer ) 
```shell
admin_addPeer enode:... 
```
